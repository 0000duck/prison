define(function(require){	
	...
	$(".bottom").find(".save").click(function(){
		...
		if(handleType == "add"){
			...
			var request = [];
			var startDateN = model.schedule.scd_start_date.replace("-","").replace("-","").replace("-","");
			var endDateN = model.schedule.scd_end_date.replace("-","").replace("-","").replace("-","");
			...
			for(var i=0;i<list.length;i++){
				request.push({
					...
					params:[{'scd_cus_number': loginUser.cusNumber,'scd_start_date': model.schedule.scd_start_date,
					         'scd_end_date': model.schedule.scd_end_date,'scd_mode_idnty': modeIdnty,
