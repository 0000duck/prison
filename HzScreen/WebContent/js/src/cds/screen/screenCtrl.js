...
//	loginUser.dept_id = 3023;
	...
	function layoutClick(){
		...
		$("#bottomScreen #layout_div .base").dblclick(function(){
			...
			var screenId = s_layout.find("#screenId").val();
			var sceneId = s_layout.find("#sceneId").val();
			layoutUpScreen(screenId,sceneId);
		})
	}
	/**
	 * 设置大屏参数
...
//	 * 布局上屏
//	 */
//	function layoutUpScreen(screenId,sceneId){
//		var url = "screen/layoutSwitch";
//		var args = {"cusNumber":loginUser.cusNumber,"screenID":screenId,"layoutID":sceneId,"remark":''};
//		//成功回调
...
//	}
	...
	$("#editScene").click(function(){
		...
		var request=[{
			...
			params:{ssn_scene_name:vm.screenScene.ssn_scene_name,ssn_cus_number:loginUser.cusNumber,ssn_screen_id:s_layout.find("#screenId").val(),ssn_scene_id:s_layout.find("#sceneId").val()}
		}];
		tip.saving();
		db.updateByParamKey({
			request:request,
	 ...
	 * 摄像机上屏
	 */
	function upScreen(screenId,cameraId,screenArea){
		var body = {
				 screenID : screenId,
				 cameraID : cameraId,
		...
		var menu = new BootstrapMenu('#baseScreen', {
		      actions: [{
		        ...
		        onClick: function(event,row) {
		        	...
		        	var screenArea = [s_top*s_height,s_left*s_width,s_width,s_height];
		        	upScreen(screenid,cameraId,screenArea);
		        },
		        isEnabled: function(row) {
		        	return isUpScreen;
		        }
...
//			        	vm.mergeCameraId = cameraId;
	 ...
	 */
	function layoutUpScreen(screenId,sceneId){
		var body = {"screenID":screenId,"layoutID":sceneId,"remark":''};
		//成功回调
		function success(data){
