define(function(require){	
	...
	$("#groupbtn").find(".add").click(function(){
		...
		dialog.open({targetId:'add_edit_panel',title:'新增大屏',h:"390"});
	})
	/**
	 * 编辑大屏信息
	 */
	$("#groupbtn").find(".edit").click(function(){
		...
		dialog.open({targetId:'add_edit_panel',title:'编辑大屏',h:"390"});
	})
	/**
	 * 删除
	 */
	...
	$("#add_edit_panel").find(".save").click(function(){
		...
		if(isAdd){
			...
			var request=[{
 				...
 				params:[{sbi_screen_name:model.screen.sbi_screen_name,
	...
	function validate(){
		var flag = true;
		if(isNaN(model.screen['sbi_screen_id'])){
			tip.alert("大屏编号必须为数字");
			flag = false;
		}else if(!model.screen['sbi_screen_name']){
			tip.alert("请输入大屏名称");
			flag = false;
