...
//				   me.bodyClick(layero);
				   layero.css('z-index',3);
				   layero.css('background','rgba(1,26,53,0.9)');
    	   ...
    	   me.index = layer.open({
			   ...
			   maxmin:obj.maxmin,
			   area: [obj.w,obj.h],
			   content:[ctx+obj.url,'yes'],
			   zIndex:1990,
			   success: function(layero, index){
//				   var body = layer.getChildFrame('body', index);
//				   console.log(layero);
//				   body.bind('click','.min',function(e){
//					   console.log(e.target);
//					   layero.find('.layui-layer-min').trigger('click');
//				   })
				   var win =layero[0].children[1].children[0].contentWindow;

				   win.onload=obj.callback;

				   //权限控制按钮
				   obj.params&&(win.params = obj.params);//传参数到弹出的iframe中的window对象中
				   me.bodyClick(layero);
			   },
			   cancel:function(){
				   if(obj.id == 293){
	...
	try {
		...
		try {
			...
			if (hz.dialog) {
				...
				hz.dialog = dialog;			// 初始化模块dialog对象
