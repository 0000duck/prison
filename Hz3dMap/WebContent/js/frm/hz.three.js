...
define(function(require) {
    ...
    HzScene.prototype.init = function(id, options) {
        ...
        // 初始化透视投影相机对象
        camera = new THREE.PerspectiveCamera(65, width / height, 100, 5000000);
        ...
        // 初始化环境光
        ambientLight = new THREE.AmbientLight('rgb(215,215,215)', 0.85);
        scene.add(ambientLight);
        //	scene1.add(ambientLight);

        // 初始化半球光源
        var hemiLight = new THREE.HemisphereLight('rgb(235,245,255)', 'rgb(235,245,255)', 0.4);
        hemiLight.color.setHSL(0.1, 0.09, 1);
        hemiLight.groundColor.setHSL(0.1, 0.09, 1);
        hemiLight.position.set(-40000, 45000, 40000);

        scene.add(hemiLight);

        // 初始化主平行光
        dirLight = new THREE.DirectionalLight('rgb(235,245,255)', 0.25);
        dirLight.position.set(-1, 1, -1);
        scene.add(dirLight);
        EventConsts.light = dirLight;
        ...
        if (_options.isShadow) {
            ...
            dirLight.shadow.camera.far = 120000;
            dirLight.shadow.bias = -0.0001;
            dirLight.shadow.mapSize.width = 8192;
            dirLight.shadow.mapSize.height = 8192;
            dirLight.castShadow = true;

            dirLight.position.set(40000, 45000, -40000);
            dirLight.color.setHSL(0.58, 0.09, 0.8);
        ...
        // 初始化辅助反面平行光
        dirLight = new THREE.DirectionalLight('rgb(235,245,255)', 0.45);
        dirLight.position.set(-1, 0, 0);
        scene.add(dirLight);

        // 初始化地面辅助平行光
        dirLight = new THREE.DirectionalLight('rgb(235,245,255)', 0.35);
        ...
        dirLight = new THREE.DirectionalLight('rgb(235,245,255)', 0.4);
        ...
        hzThree.showStats = function(visible, className) {
            if (!stats) {
                ...
                stats.dom.style.cssText = "position:fixed;cursor:pointer;opacity:0.9;";
        ...
        patrol3D = new Patrol3D(camera, scene);
    ...
    hzThree.addModel = function(data, callbackHandler) {
        ...
        if (data.hasOwnProperty('position') === true) {
            modelPos = new THREE.Vector3(data.position.x, data.position.y, data.position.z);
        }
        if (data.hasOwnProperty('rotation') === true) {
            modelRotation = new THREE.Vector3(data.rotation.x, data.rotation.y, data.rotation.z);
        }
    ...
    hzThree.isInBox = function(p, boxname) {
        let v = new THREE.Vector3(parseFloat(p.x), parseFloat(p.y), parseFloat(p.z));
        let b = false;
        let box = hzThree.getObjectByName(boxname);
        if (box === undefined || box === null) {
            return true;
        }
        if (box instanceof THREE.Object3D) {
            box.geometry.computeBoundingBox();
            let box3 = box.geometry.boundingBox;
