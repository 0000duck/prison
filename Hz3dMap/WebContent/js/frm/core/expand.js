    ...
    var expand = function() {
        ...
        THREE.Object3D.prototype.showYArrow = function(value = true, color = 0xFF0000, radius = 1, bottomRadius = 3, bottomColor = 0x00FFFF) {
            if (value === true) { //展示向下的箭头
                ...
                let result = raycast.intersectObjects(scene.children, true);

                if (result.length > 0) {
                    let pos = result[0].point;
                    let line = new DiyLine(this.position.clone(), pos, radius, color);
                    if (this.parent !== undefined) {
                        this.parent.add(line);
                        this.yline = line;

                        let geometry = new THREE.SphereBufferGeometry(bottomRadius, 32, 32);
                        let material = new THREE.MeshBasicMaterial({ color: bottomColor });
        ...
        THREE.Object3D.prototype.updateYArrow = function() {
            if (this.yline !== undefined) {
                ...
                let radius = this.yline.radius;
                let color = this.yline.color;
                this.yline.dispose();

                let pos0 = this.position.clone();                
                pos0.y -= 100;
