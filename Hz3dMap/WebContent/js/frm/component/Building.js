    ...
    var Building = function(buildName) {
        ...
        function recordData(cellName, cell, faceforward, sphere, box) {
            ...
            if (scope.buildCellMap.hasOwnProperty(cellName) === true) {
                ...
                if (faceforward === 0) { //内部
                    scope.buildCellMap[cellName].inside = { object: cell, sphere: sphere, box: box };
                } else {
                    scope.buildCellMap[cellName].outside = { object: cell, sphere: sphere, box: box };
                }
            } else {
                var obj = {};
                obj.side = [];
        ...
        this.separate = function() {
            ...
            for (var key1 in scope.buildCellMap) {
                ...
                if (minHeight === 0 || minHeight > diffHeight1) {
            ...
            for (var key in scope.buildCellMap) {
                if (key.indexOf('01') === -1) { //一楼不用管它
                    ...
                    for (var index = 0; index < cellArray.length; index++) {
                        ...
                        var disObj = { x: disX, y: disY, z: disZ };
                        separateCellDataKey[key] = { x: tempValue.x, y: diffHeight, z: tempValue.z };
