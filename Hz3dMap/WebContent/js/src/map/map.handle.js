...
define(function (require) {
	...
	function HzMap () {
		...
		this.cusViewMenu = null;	// 当前视角菜单数据
...
//				this._transformObj.setBorderShow(false);
	...
	HzMap.prototype._visibleRoomModel = function (visible) {
		var list = this.hzThree.objectMap['T016'] || [], room;

		for(var i = 0; i < list.length; i++) {
			room = list[i];
			room.visible = visible;

			hzEvent.emit('hz.room.model.visible', room.name, visible);
		}
	}
	...
	HzMap.prototype.getModelPointByLinkId = function (linkType, linkId) {
		...
		var temp = null, params = null;

		for(var i = 0; i < array.length; i++) {
			temp = array[i];
			params = temp.hzParams;
			if (params.type == linkType && params.linkId == linkId) {
				return hzThree.getObjectByName(temp.name);
			}
		}
		return null;
	}
	...
	HzMap.prototype.removeModelPointByLinkId = function (linkType, linkId) {
		...
		for(var i = 0; i < array.length; i++) {
			...
			if (temp.type == linkType && temp.linkId == linkId) {
				hzThree.removeModelByName(temp.name);
				array.splice(i, 1);
			}
		}
	}
	...
	function loadViewMenu (hzMap) {
		...
		if (user.dataAuth == 0) {
			whereId = 3;
			params = [user.cusNumber, user.cusNumber, user.deptId];
	...
	function loadModelPoint (hzMap, loadNo, viewId) {
		...
		db.query({
			request: {
				sqlId: 'select_model_point_for_map_handle',
				whereId: 1,
				params: [user.cusNumber, viewId, FLAG]
			},
			success: function (array) {
			...
			success: function (powerGrids) {
				...
				db.query({
					...
					success: function (points) {
						var temp, pgid, powerGrid = hzMap.hzThree.PowerGrid;
