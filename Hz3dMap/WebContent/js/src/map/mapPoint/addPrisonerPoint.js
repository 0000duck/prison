    ...
    function(vue,db,mapHandle,tip,user,MapPrisoner,hzEvent,select){
        ...
        window.ipt_add = function () {
            ...
            for(var i=0;i<prisonerList.length;i++){
                if(model.panelPoint.ppi_link_id == prisonerList[i].id){
                    prisonerId = prisonerList[i].prisonerid;
                    prisonerName = prisonerList[i].name;
                    imgUrl = prisonerList[i].img_url;
                }
            }
            if (infoPanel == null) {
                mapPrisoner.add({
                    ...
                    'code': prisonerId + "",
                    'name': prisonerName,
                    'image': basePath + 'css/image/zfpic.jpg',
                    'position': position,
