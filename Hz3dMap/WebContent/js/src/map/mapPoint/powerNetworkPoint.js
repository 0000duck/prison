...
define(['frm/loginUser','frm/hz.db','ztree','vue','frm/treeUtil','frm/message','hz/map/map.handle','frm/select','frm/model','cxcolor','frm/hz.event'],
    function(user,db,ztree,vue,treeUtil,message,hzmap,select,model,cxcolor, hzEvent){
        ...
        var vm = new vue({
            ...
            methods:{
                ...
                backList:function(){		//返回
                    pndiv.style.animation="toright1 0.5s";
                    //关闭绘图
                    vm.closePaining();
                },
                ...
                addPoints:function(){	//添加
                    pndiv.style.animation="toleft1 0.5s";
                    //重置数据
                    resetPoint();
                },
                ...
                updateViewInfo:function(infoSqlId){		//更新添加电网图形信息
                    ...
                    var view = hzEvent.call('home.viewmenu.getCurViewMenu');
        ...
        //旋转终止
        pndiv.addEventListener("animationend",function(e){
            if(e.animationName=='toleft1'){
                $("#powerNetworkPoint").hide();
                $("#powerNetworkPointAdd").show();
                pndiv.style.animation="toleft2 0.5s";
                setWin.style.transform='rotateY(-180deg)';
                title.style.transform = 'rotateY(-180deg)';
                setWin.style.left = '15px';
                setWin.style.right = 'inherit';
            }else if(e.animationName=='toright1'){
                $("#powerNetworkPoint").show();
                $("#powerNetworkPointAdd").hide();
                pndiv.style.animation="toright2 0.5s";
                setWin.style.transform='rotateY(0deg)';
                title.style.transform = 'rotateY(0deg)';
                setWin.style.right = '15px';
                setWin.style.left = 'initial';
            }
        });
            ...
            db.query({
                ...
                success:function(data){
                    data.push({
                        ...
                        pid:0,
                        isParent:true,
                        icon:'css/images/icons/org1.png'
                    });
                    vm.treeData = data;
                    pointTree = $.fn.zTree.init($('#powerNetworkZtree'),treeSetting,data);
