		...
		methods:{	
			showCreate:function(type){
				...
				case 'create':
					...
					modelUtil.clear(this.playTask,{'utt_cus_number':'','utt_task_type':'','utt_user_id':'','userId':''});
					_title = '新建定时任务';
					break;
				case 'edit':
					this.editModel = true;
...
//					loadEditData(this.selectedRow);
			...
			addTask:function(){
				...
				if(this.editModel){
					timerTask.updateTimerTask(this.playTask,$('#table'));
				} else{
					timerTask.addTimerTask(this.playTask,$('#table'));
				}
				dialog.close();
			},
			deleteTask:function(){
				...
				timerTask.deleteTimerTask(this.selectedRow.utt_task_id,$('#table'));
			},
			resetTask:function(){
