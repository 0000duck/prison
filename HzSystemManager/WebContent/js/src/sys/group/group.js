define(['jquery','vue','frm/hz.db','ztree','frm/model','frm/message','frm/treeUtil'],function($,tpl,db,treeSelect,modelUtil,tip,util){
	...
	//分组树
	db.query({
		request:{sqlId: 'query_group_sql',whereId:2},
		success:function(data){
		
			data.push({'agd_id':'-1','agd_name':"角色分类",icon:"diy/1_close.png",open:true});
		...
		if(this.textContent=="保存"){
 		...
 		if(this.textContent=="删除"){
			...
			var list=treeContainer.transformToArray(node).map(function(row){
 			...
 			tip.confirm(list.length>1?"确认删除该角色分组及其子节点？":"确认删除该角色组？",function(){
 				db.updateByParamKey({
 	 				request:{
	...
	function validate(){
		return !model.group['agd_name'].length&&!tip.alert("请输入角色分类名称")||
				model.group['agd_name'].length>26&&!tip.alert("分类名过长，请输入26个以内字符")||
			   !model.group['agd_key'].length&&!tip.alert("请输入角色分类标识");
 	}
});
