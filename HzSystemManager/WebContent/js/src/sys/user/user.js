	...
	formContainer.on("click",".buttons a",function(){
		...
		if(this.textContent=='保存'){
			...
			if(model.user['id'].length){//修改
				...
				$.post(top.ctx+'sys/update',param,function(){
					pflag=false;
					
					var list=roles.map(function(row){
	 						return {'urr_cus_number':model.user['ubd_cus_number'],'urr_user_id':model.user['id'],'urr_role_id':row.id.replace("1-","")};
	 					});
					db.updateByParamKey({
						request:[{
				...
				db.query({
					...
					success:function(data){
						...
						$.post(top.ctx+'sys/update',"userInfo="+JSON.stringify(request)+"&name="+model.user['name']+"&password="+model.user['ubd_pass'],function(data){
							pflag=false;
							data=JSON.parse(data);
							var userid=data.data.data[0]['seqList'][0];
							
							var list=roles.map(function(row){
		 	 						return {'urr_cus_number':model.user['ubd_cus_number'],'urr_user_id':userid,'urr_role_id':row.id.replace("1-","")};
		 	 					});
							db.updateByParamKey({
								request:[{sqlId:'insert_user_role',params:list},{sqlId:'insert_person_set',params:[{cus:model.user['ubd_cus_number'],user:userid,pwd:'000000',avoid:'0'}]}],
								success:function(){
			 		 				var  temp=model.user;
			 		 				temp.icon="user.png";
			 		 				temp.id=userid;
