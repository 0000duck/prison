define(['jquery','vue','frm/hz.db',"frm/table","frm/dialog","frm/message","frm/model","frm/select","frm/loginUser"],function($,vue,db,table,dialog,message,modelData,select,user){
	...
	var model=new vue({
		...
		watch:{
			'rfidBind.rpb_people_type':function(){
				if(flag){
					flag = false;
				}else{
		...
		methods:{
			selectKey:function(item){
				...
				model.rfidBind.rpb_people_name=item.person_name;
	...
	table.init("table",{
		...
		columns: [[  
         ...
         onClickRow:function(row){
        	 ...
        	 dialog.open({targetId:'operPanel',title:'修改绑定',h:"300"}); 
         }
	});
