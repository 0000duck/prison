...
function(chart,tpl,login,db,tip,tree,modelUtil,video,mapHandle,ls,time){
	...
	var imgurl = basePath + 'css/image/zfpic.jpg';
	var param= param ? param : ls.getItem("ckParam");
	var prisonerId = param && param.id ? param.id : "";
	var prisonerName = param && param.name ? param.name : "";
		...
		db.query({
			...
			success: function (result) {
				if (result && result.length) {
					var temp = null, linkInfo, playbackList = [], playback;
					...
					for(var i = 0; i < dvcList.length; i++) {
						...
						for(var j = 0; j < result.length; j++) {
							linkInfo = result[j];
							if (temp.rfid == linkInfo.dlm_dvc_id) {
								if (linkInfo.dld_dvc_type == 1) {
									playback = getPlaybackParam([linkInfo.dld_dvc_id], temp.time);
									playbackList = playbackList.concat(playback);
								}
							}
						}
					}
...
//				player(lastData.cameraIds, lastData.time);
	...
	function rfidTrackMove (list) {
		var curViewMenu = mapHandle.cusViewMenu || {};
		...
		(function findAllPath () {
			if (curWayObj) {
				var rpoA = getRfidPointObj(list[index++].rfid_id);
				var rpoB = getRfidPointObj(list[index].rfid_id);

				if (rpoA && rpoB) {
					curWayObj.findPath(rpoA.position.clone(), rpoB.position.clone(), function (meshName, points) {

						if (index > 1) points.shift();
	...
	function getPlaybackParam (ids, datetime) {
		...
		if (ids && ids.length) {
			...
			var startTime = new Date(time.setSeconds(time.getSeconds()-1));
			var endTime = new Date(time.setSeconds(time.getSeconds()+2));
