...
public class AlarmMessageService {
    ...
    public JSONObject alarmProcess(String cusNumber,FeAlarmMessageBean feAlarmMessageBean){
    	JSONObject rtnObj=new JSONObject();
    	rtnObj.put("alarmRecord", feAlarmMessageBean);
    	String alarmDeviceType=feAlarmMessageBean.getAlarmDeviceType();
    	String alarmId=feAlarmMessageBean.getAlarmID();
    	//查询关联设备
		JSONObject queryJSON = null;
		try {
			queryJSON = new JSONObject();
			queryJSON.put(ReqParams.SQL_ID, "select_alt_link_dvc_rltn");
			queryJSON.put(ReqParams.WHERE_ID, "1");
			queryJSON.put(ReqParams.ORDER_ID, "1");
			List<String> paraList=new ArrayList<String>();
			paraList.add(cusNumber);
			paraList.add(alarmDeviceType);
			...
			//获取地图视角
			Map<String, Object> viewMenuMap=queryViewMenuInfo(cusNumber,alarmDeviceType,alarmId);
			rtnObj.put("view", viewMenuMap);
			//获取在监民警
			String departmentId=alarmBaseDtlsCache.getAlarmDepartmentId(cusNumber, alarmId, alarmDeviceType);
			if(!departmentId.equals("")){
				//获取在监民警
				List<Map<String, Object>> policeArray=getPoliceInfo(cusNumber,departmentId);
				rtnObj.put("police", policeArray);
				//获取重点罪犯
				List<Map<String, Object>> prisonerArray=getPrisonerInfo(cusNumber,departmentId);
				rtnObj.put("prisonerArray", prisonerArray);
			}
		} catch (Exception e) {
    ...
    private List<Map<String, Object>> getPrisonerInfo(String cusNumber,String departmentId){
    	try {
			JSONObject queryJSON = new JSONObject();
			queryJSON.put(ReqParams.SQL_ID, "select_prisoner_base_dtls");
			queryJSON.put(ReqParams.WHERE_ID, "1");
			List<String> paraList=new ArrayList<String>();
			paraList.add(cusNumber);
    ...
    private List<Map<String, Object>> getPoliceInfo(String cusNumber,String departmentId){
    	try {
			JSONObject queryJSON = new JSONObject();
			queryJSON.put(ReqParams.SQL_ID, "select_in_prisoner_police_info");
			queryJSON.put(ReqParams.WHERE_ID, "1");
			List<String> paraList=new ArrayList<String>();
			paraList.add(cusNumber);
			paraList.add(departmentId);
...
/*    *//**
     * 获取报警设备所属部门
     * @param cusNumber
     * @param alarmDeviceType 报警设备类型
     * @param deviceId 设备编号
     * @return
     *//*
    private String getDepartmentId(String cusNumber,int alarmDeviceType,String deviceId){
    	String departmentId="";
		if(alarmDeviceType==DeviceTypeConst.CAMERA_TYPE){
			departmentId=getCameraInfo(cusNumber,deviceId,"cbd_dept_id");
		}else if(alarmDeviceType==DeviceTypeConst.DOOR_TYPE){

		}else if(alarmDeviceType==DeviceTypeConst.TALK_TYPE){
			
		}else if(alarmDeviceType==DeviceTypeConst.BROADCAST_TYPE){
			
		}else if(alarmDeviceType==DeviceTypeConst.SCREEN_TYPE){
			
		}else if(alarmDeviceType==DeviceTypeConst.NETWORK_ALARM_TYPE){
