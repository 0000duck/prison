define(['jquery','vue','frm/hz.db',"frm/table","frm/dialog","frm/message","frm/model","frm/loginUser","frm/select"],
		...
		table.init("table",{
			...
			columns: [[  
	         ...
	         onClickRow:function(row){
	        	 ...
	        	 dialog.open({targetId:'updateHandleFlowItem',title:'编辑',h:"300"});
	         }
		});
	}
	...
	$("#handleFlowGroupBtn").on("click","a",function(){
		if(this.textContent=="添加"){
			dialog.open({targetId:'updateHandleFlow',title:'新增',h:"300"});
		...
		if(this.textContent=="删除"){
			...
			isAddFlag = false;
			dialog.open({targetId:'updateHandleFlow',title:'编辑',h:"300"});
		}
	});
	...
	$("#handleFlowItemGroupBtn").on("click","a",function(){
		if(this.textContent=='添加'){
			...
			dialog.open({targetId:'updateHandleFlowItem',title:'新增',h:"300"});
		}else{//流程项删除
