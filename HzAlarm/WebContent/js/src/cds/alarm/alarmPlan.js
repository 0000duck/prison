define(function(require){
	...
	// 默认显示的报警设备树的索引
	var defaultShowAlarmDvc = [0,1,3,5,9,14];
...
//						}
	...
	var mmt = {
			...
			typeFlag: 'link_type_',
			types: [],		// 关联的设备类型集合
			teypsTemp: [],	// 临时对象用来存储编辑前的数据

			dvcFlag: 'link_dvc_',
			dvcs: [],		// 关联的设备集合
			dvcsTemp: [],	// 临时对象用来存储编辑前的数据
			...
			edit: function () {
				if (!this.isEdit) {
					this.isEdit = true;
					this.typesTemp = [].concat(this.types);
					this.dvcsTemp = [].concat(this.dvcs);
				}
			},
			 ...
			 */
			cancel: function () {
				var alertors = this.alertors;
				var types = this.typesTemp;
				var dvcs = this.dvcsTemp;
				var temp = null;
				...
				if (this.isEdit) {
					...
					for(var i = 0; i < alertors.length; i++) {
						temp = alertors[i].data;
						this.addAlertor(temp.zNode, temp.showClose, temp.hasDelete);
					}

					mmt.removePlus();
					for(var i = 0; i < types.length; i++) {
						temp = types[i];
						this.addType(temp.id, temp.text, temp.data);
					}
					mmt.addPlus();

					for(var i = 0; i < dvcs.length; i++) {
						temp = dvcs[i];
						this.addDvc(temp.type, temp.id, temp.text, temp.data);
					}
				}
			},
			 ...
			 * 添加报警器节点
			 */
			addAlertor: function (node, showClose, hasDelete) {
				var mmNode = {
					id: node.type + '_' + node.id, 
					text: node.name, 
					type: NODETYPE.ALERTOR, 
					showClose: showClose, 
					data: {
						zNode: node,
						showClose: showClose,
				...
				if (mindMap.addRoot(mmNode)) 
					this.alertors.push(mmNode);

				if (this.alertors.length == 1) 
					this.addPlus();
			},
			 ...
			 * 删除根节点
			 */
			removeAlertor: function (node) {
				var id = node.type + '_' + node.id;
				mindMap.removeRoot(id);
				this.removeArray(this.alertors, id);
				if (this.alertors.length == 0)
					this.removePlus();
			},
			 ...
			 */
			addPlus: function () {
				mindMap.addChild({id:'addLinkType', text:'+', type: NODETYPE.PLUS, className: 'add', 'title': '点击添加关联设备!', showClose: false});
			},
			 ...
			 */
			removePlus: function () {
				mindMap.removeChild('addLinkType');
			},

			/*
			 * 添加子节点
			 */
			addNode: function (type, id, text, attrs) {
				var typeName = null;
				for(var i = 0; i < vm.linkTypes.length; i++) {
					if (type == vm.linkTypes[i].id) {
						typeName = vm.linkTypes[i].name;
					}
				}

				this.addType(type, typeName, attrs);
				this.addDvc(type, id, text, attrs);
			},

			/*
			 * 删除子节点
			 */
			removeNode: function (type, id) {
				this.removeDvc(type, id);
				this.hasDvcs(type) || this.removeType(type);
			},
			 ...
			 * 添加关联类型节点
			 */
			addType: function (id, text, data) {
				if (!this.hasType(id)) {
					var typeData = {id: id, text: text, data: data};
					var nodeData = {'id': this.typeFlag + id, 'text': text, type: NODETYPE.LINKTYPE, 'data': typeData};

					if (mindMap.addChild(nodeData)) {
			...
			removeType: function (type) {
				for(var i = 0; i < this.types.length; i++) {
					if (this.types[i].id == type) {
						mindMap.removeChild(this.typeFlag + type);
			 ...
			 * 是否还有关联设备
			 */
			hasDvc: function (type, id) {
				var dvc = null;
				for(var i = 0; i < this.dvcs.length; i++) {
					dvc = this.dvcs[i];
					if (dvc.type == type && dvc.id == id) {
						return true;
					}
				}
				return false;
			 ...
			 * 是否还有关联设备
			 */
			hasDvcs: function (type) {
				for(var i = 0; i < this.dvcs.length; i++) {
					if (this.dvcs[i].type == type) {
						return true;
					}
				}
				return false;
			 ...
			 * 添加设备节点
			 */
			addDvc: function (type, id, text, data) {
				if (!this.hasDvc(type, id)) {
					var pid = this.typeFlag + type;
					var nid = this.dvcFlag + type + id;
					var dvcData = {id: id, text: text, type: type, data: data};
					var nodeData = {'pid': pid, 'id': nid, 'text': text, type: NODETYPE.LINKDVC, 'className':'small', 'data': dvcData};

					if (mindMap.addChild(nodeData)) {
						this.dvcs.push(dvcData);
						return nodeData;
					}
				}
				return null;
			 ...
			 * 删除设备节点
			 */
			removeDvc: function (type, id) {
				utils.each(this.dvcs, function (dvc, i) {
					if (dvc.type == type && dvc.id == id) {
						mindMap.removeChild(mmt.dvcFlag + type + id);
						mmt.dvcs.splice(i, 1);
						return true;
					}
				});
	...
	function loadDeviceTypes(){
		db.query({
			request: {
				sqlId:'select_constant_bycode', whereId:0, params:['LDR_ALERTOR_TYPE']
			},
			success: function (data) {
	...
	function loadLinkTypes(){
		db.query({
			request: {
				sqlId:'select_constant_bycode', whereId:0, params:['LDR_DVC_TYPE']
			},
			success: function (data) {
				vm.linkTypes = data;
				utils.each(data, function (type) {
					var sqlId = linkDvcSql[type.id];
					if (sqlId) {
						db.query({
							request: {sqlId: sqlId, whereId: 1, orderId: 1, params: [user.cusNumber]},
							success: function (data) {
								if (data && data.length) {
									vm.cacheDvcs(type.id, data);
								}
							}
						});
					}
